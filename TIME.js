...
if (Vs > Vz && Xe < ((1.8 * ((Vs * Vs) - (Vf * Vf)))/(3.6 * Ag + S / K)) + Vh * Fr){
  alert ('所定キョリ内で終速度まで減速できません！再読み込みして初速度をもう少し低く設定するか、次の区間と統合するなどしてください。');
}
...
while (Xk + Xg > Xe){
  ...
  if (Vh < Vs || Vh < Vf){
    ...
    if (Xk + Xg <= Xe){
      ...
      alert ('再読み込みして値を設定しなおすか、次の区間と統合するなどしてください。');
    }
  }
}
